...
def _load_free_grids(paths=FREE_GRIDS_CANDIDATES):
    free = set()
    for path in paths:
...
def _priorities_for_block_direction(block_direction: str) -> list[str]:
    ...
    Order is chosen to move away from the blocking sector while keeping the active robot's path clear.
    """
    if block_direction == "front":
        # move behind and to a side first
...
# ----------------- CLASSIC FUNCTIONS (unchanged logic, plus file check) -----------------

def _make_candidate(curx: float, cury: float, direction: str, margin: float):
    """
...
_PROBE_VEC = {
    "left":(+0.1,0.0), "right":(-0.1,0.0), "front":(0.0,-0.1), "back":(0.0,+0.1),
    "front-left":(+0.1,-0.1), "front-right":(-0.1,-0.1),
    "back-left":(+0.1,+0.1),  "back-right":(-0.1,+0.1),
}

def _dir_status_free(dirs, want_dir: str) -> bool:
    x_status, y_status, frdiag_status, brdiag_status = dirs
    if want_dir == "left":         return x_status != "left"
    if want_dir == "right":        return x_status != "right"
    if want_dir == "front":        return y_status != "front"
    if want_dir == "back":         return y_status != "back"
    if want_dir == "front-left":   return frdiag_status != "front-left"
    if want_dir == "front-right":  return frdiag_status != "front-right"
    if want_dir == "back-left":    return brdiag_status != "back-left"
    if want_dir == "back-right":   return brdiag_status != "back-right"
    return True
...
def _dir_is_free_with_new_api(robot_name: str, test_dx: float, test_dy: float, want_dir: str) -> bool:
    ...
    # free only if that exact sector is clear AND no robot is reported in that exact sector
...
def _vector_to_dir8(dx: float, dy: float) -> str:
    ...
    ax, ay = abs(dx), abs(dy)
    # favor diagonals unless one axis dominates by 1.5x
    DIAG_RATIO = 1.5
    if ax >= DIAG_RATIO * ay:
...
async def find_nearest_free_spot(current_position, block_direction, toward_goal, blocking_robot):
    ...
    # Choose a small intent vector for probing that direction with sensors
    intent_vectors = {
        "left":        ( +0.1,  0.0),
        "right":       ( -0.1,  0.0),
        "front":       (  0.0, -0.1),
    ...
    for d in try_order:
        ...
        # 2) build candidate point in that direction (respecting your "keep x for back, etc.")
...
async def request_robot_to_clear(sim, toward_goal, active_robot, blocking_robot, block_direction):
    ...
    if temp_goal:
        ...
        REACH_EPS = 0.12
        # keep the original block_direction as the "cause"; we may replan using it
        cause_dir = block_direction
        ...
        while True:
            ...
            move = "Diagonal" if (abs(dx) > 0.05 and abs(dy) > 0.05) else \
